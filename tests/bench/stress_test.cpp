...
static void run_mixed_workload(const fs::path &path)
{
    ...
    auto worker = [&](int thread_id) {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            if (op < 60) {
                ...
                int v1 = val_dist(rng);
                int v2 = val_dist(rng);
                auto q = "INSERT INTO users VALUES (" + std::to_string(v1) + ", " +
                         std::to_string(v2) + ")";
                if (exec.execute(q))
                ...
                // Actually our UPDATE without WHERE updates everything. Let's just insert for now
                // to avoid huge table scans in updates blocking everything. Wait, UPDATE users SET
                // balance = X updates the ENTIRE TABLE. With 16 threads, it will OOM or block! We
                // should add WHERE clause? Our Parser currently supports simple WHERE? Let's just
                // do INSERT and DELETE all. Wait, our DELETE doesn't support WHERE yet. Let's just
                // focus on concurrent INSERTS, and let one thread periodically DELETE ALL to test
                // GC.
            }
            else {
                // 20% SELECT
    ...
    // Instead of random UPDATE which updates the entire table, let's have 15 inserter threads and 1
    // deleter thread.
    auto inserter = [&](int thread_id) {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            int v2 = val_dist(rng);
            auto q =
                "INSERT INTO users VALUES (" + std::to_string(v1) + ", " + std::to_string(v2) + ")";
            if (exec.execute(q))
                total_inserted.fetch_add(1, std::memory_order_relaxed);
            conn.commit();
        }
