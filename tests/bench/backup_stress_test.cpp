...
int main()
{
    ...
    auto inserter_worker = [&](int thread_id) {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            conn.begin();
            // Batch of 100 inserts per transaction to stress WAL
            for (int i = 0; i < 100; ++i) {
                std::stringstream ss;
    ...
    std::cout << "Started " << kInserters << " inserter threads, " << kSelectors
              << " selector threads, and 1 manual backup thread." << std::endl;
    ...
    for (int i = 0; i < kDurationSec; ++i) {
        ...
        std::cout << "Time elapsed: " << i + 1 << "s / " << kDurationSec
                  << "s (Inserts: " << total_inserts.load() << ", Selects: " << total_selects.load()
                  << ")" << std::endl;
    }
