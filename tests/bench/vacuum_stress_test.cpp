...
int main()
{
    ...
    constexpr int kInserters = 5;
    constexpr int kSelectors = 3;
    constexpr int kUpdaters = 5;
    constexpr int kDeleters = 5;
    ...
    std::atomic<int64_t> total_rollbacks{0};
    std::atomic<int64_t> total_vacuums{0};
    ...
    auto inserter_worker = [&](int thread_id) {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            // 10% chance to rollback
            if (val_dist(rng) % 10 == 0) {
                conn.rollback();
                total_rollbacks.fetch_add(1, std::memory_order_relaxed);
            }
            else {
                conn.commit();
                total_inserts.fetch_add(successful_inserts, std::memory_order_relaxed);
            }
        }
    };
    ...
    auto selector_worker = [&](int thread_id) {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            conn.begin();
            // Full table scan without index to ensure we touch all pages while VACUUM runs
            auto res = exec.execute("SELECT id FROM stress WHERE value > 500000");
            if (res) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(
                500)); // Sleep to allow heavy full table scans not to monopolize
        }
    };
    ...
    auto vacuum_worker = [&]() {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            std::cout << "[Vacuum Thread] Starting VACUUM...\n";
            conn.begin();
            if (exec.execute("VACUUM stress")) {
                std::cout << "[Vacuum Thread] VACUUM SUCCESS.\n";
                total_vacuums.fetch_add(1, std::memory_order_relaxed);
            }
            else {
                std::cout << "[Vacuum Thread] VACUUM FAILED.\n";
                errors.fetch_add(1, std::memory_order_relaxed);
            }
            conn.commit();
        }
    ...
    std::cout << "Total Rollbacks: " << total_rollbacks.load() << "\n";
    std::cout << "Total Vacuums: " << total_vacuums.load() << "\n";
