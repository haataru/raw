...
auto TransactionManager::commit(std::shared_ptr<Transaction> txn, Database &db) -> Status
{
    ...
    if (auto s = db.wal().append_commit(txn->tx_id, commit_ts); !s) {
        return s.error();
    }
...
auto TransactionManager::rollback(std::shared_ptr<Transaction> txn, Database &db) -> Status
{
    ...
    // We set their ts to 0 (which is less than any read_ts, wait, 0 is visible to everyone! No, 0
    // is a tombstone timestamp? Wait, if it's 0, it's older than any snapshot, so it's visible if
    // it's an insert! We should set it to `kTxIdFlag` without any tx_id, meaning it's aborted, or a
    // special tombstone value. Or we can just set it to `std::numeric_limits<Timestamp>::max()` so
    // it's from the infinite future and never visible! Yes! Infinite future.
