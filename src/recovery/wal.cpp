...
auto WalWriter::open(const std::filesystem::path &db_path) -> Status
{
    ...
    std::snprintf(name_buf,
                  sizeof(name_buf),
                  "%020llu.log",
                  static_cast<unsigned long long>(current_segment_start_lsn_));
...
auto WalWriter::append_record(TxId tx_id,
                              WalRecordType type,
                              const std::vector<std::byte> &payload) -> StatusOr<Lsn>
{
    ...
    if (file_.fail() || file_.bad()) {
        ...
        return std::unexpected(
            Status{Status::kNoSpace, "failed to append wal record: no space left on device"});
    }
...
auto WalWriter::append_commit(TxId tx_id, Timestamp commit_ts) -> StatusOr<Lsn>
{
    std::vector<std::byte> payload(sizeof(commit_ts));
    std::memcpy(payload.data(), &commit_ts, sizeof(commit_ts));
    return append_record(tx_id, WalRecordType::kCommit, payload);
}
...
auto WalWriter::append_insert(TxId tx_id,
                              TableId table_id,
...
auto WalWriter::append_insert_batch(TxId tx_id,
                                    TableId table_id,
...
auto WalWriter::append_delete(TxId tx_id,
                              TableId table_id,
...
auto WalWriter::append_full_page(TableId table_id,
                                 PageId page_id,
                                 int32_t file_id,
                                 const std::byte *data,
...
{
    std::vector<std::byte> payload;
    payload.reserve(sizeof(table_id) + sizeof(page_id) + sizeof(int32_t) + size);
    ...
    put(&table_id, sizeof(table_id));
    put(&page_id, sizeof(page_id));
    put(&file_id, sizeof(file_id));
    put(data, size);

    return append_record(kInvalidTxId, WalRecordType::kFullPageImage, payload);
}
