...
auto VersionIndex::search(RowId row_id,
                          Timestamp max_ts,
                          TxId current_tx_id) const -> StatusOr<uint64_t>
{
    ensure_sorted();
    ...
    for (; it != entries_.end() && it->row_id == row_id; ++it) {
        uint64_t raw_ts =
            reinterpret_cast<const std::atomic<uint64_t> &>(it->ts).load(std::memory_order_acquire);
